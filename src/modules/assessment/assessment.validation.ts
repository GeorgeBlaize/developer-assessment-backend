...
const windowRefinement = (data: { startWindow?: Date; endWindow?: Date }, ctx: z.RefinementCtx) => {
  if (data.startWindow && data.endWindow && data.endWindow <= data.startWindow) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: 'endWindow must be after startWindow',
      path: ['endWindow'],
    });
  }
};
